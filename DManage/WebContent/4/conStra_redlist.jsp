...
<head>
    ...
    <script type="text/javascript">
        ...
        function del(i,name,grouptype){
            ...
            document.forms["mod"+i].action="conStra_redlist_del.jsp";
        ...
        function mod(ls_type,goupname,goupid,grouptype,school,loginname){           
            parent.frames['main'].location="conStra_redlist_edit.jsp?city_type="+ls_type+"&goupname="+goupname+"&id="+goupid+"&school="+school+"&loginname="+loginname;
        }
...
<table width="99%"  align="center" border="0" cellpadding="0" cellspacing="1" class="tableform">
    ...
    <form  method="post" target="actions" name="mod<%=i%>">
        ...
        <tr bgcolor="#ffffff" > <input type="hidden" name="goupid" value="<%=ht.get("groupid")%>">
            ...
            <td align="center" class="block" height="30"><input type="hidden" name="loginname" class="b9" size="30" value="<%=loginname%>"><%=loginname%>
            </TD>

            <td align="center" width="180px">
                <table width="100%">
                    <tr>
                        <td onclick="mod('<%=citycode%>','<%=groupname%>','<%=ht.get("id")%>','<%=grouptype%>','<%=schoolid%>','<%=loginname%>')" align="center">
        ...
        <td  class="b9" colspan="6" height="20">
            <table width="100%" height="12">
                <tr>
                    ...
                    <td class="b9" align="right"  valign="middle"><%=Tools.getPageString(users.size(),iPageSize, iPageNum,"conStra_redlist.jsp"+"?"+para+"&")%></td>
                </tr>
            </table>
        </td>
    </tr>
