...
<head>
    ...
    <script type="text/javascript">
        ...
        function modadvancesearch(ls_type,goupname,goupid,grouptype,school,osid,accessid,loginname) {
            if(grouptype==1){
                alert("本功能不支持红名单组的修改！");
                return false;
            }
            parent.frames['main'].location="version_grep_edit.jsp?city_type="+ls_type+"&goupname="+goupname+"&id="+goupid+"&grouptype="+grouptype+"&school="+school+"&osid="+osid+"&accessid="+accessid+"&loginname="+loginname+"&grouptypeold="+grouptype;
        }
...
<form  method="post" target="actions" name="mod<%=i%>">
    ...
    <tr bgcolor="#ffffff" > <input type="hidden" name="goupid" value="<%=ht.get("groupid")%>">
        ...
        <td align="center"  height="30" class="block">
            <input type="hidden" name="osid" value="<%=osid%>" class="table_normal" >
            <%=ht.get("osname")%></td>
        <td align="center" class="block" height="30">

            <input type="hidden" name="accessid" class="table_normal" value="<%=accessid%>">
           <%=accessname%>
        </td>
        ...
        <td align="center" width="180px">
            <table >
                <tr>
                    <td onclick="modadvancesearch('<%=citycode%>','<%=groupname%>','<%=ht.get("id")%>','<%=grouptype%>','<%=schoolid%>','<%=osid%>','<%=accessid%>','<%=loginname%>')" align="center">
...
%>
<tr bgcolor="#CCCCCC">
    <td  class="b9" colspan="9" height="20">
        <table width="100%" height="12">
            <tr>
                <td class="b9" align="left" valign="middle" >总记录:<%= users.size()%> 条</td>
                <td class="b9" align="right"  valign="middle"><%=Tools.getPageString(users.size(),iPageSize, iPageNum,"version_grep.jsp"+"?"+para+"&")%></td>
            </tr>
        </table>
    </td>
</tr>
